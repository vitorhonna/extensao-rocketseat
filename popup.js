...
function runExtension() {
    ...
    const contentBox = document.querySelector(".sc-kMOkjD");

    contentBox.style.paddingTop = 0;

    const breadcrumbs = document.querySelector(".sc-kNMOeM");

    removeIfExists(breadcrumbs);

    // ---------------------------------------------

    const contentParent = document.querySelector(".sc-chCrNI");

    console.log(contentParent);

    removeIfExists(contentParent.childNodes[1]);
    hideIfExists(contentParent.childNodes[1]);
    hideIfExists(contentParent.childNodes[2]);
    hideIfExists(contentParent.childNodes[3]);
    hideIfExists(contentParent.childNodes[4]);

    /* Esconder descrição quando o próximo video for carregado */
    ...
    contentParent.style.padding = "5px 5px 5px 5px";
    contentParent.style.height = "100vh";
    // contentParent.style.backgroundColor = "white"

    // ---------------------------------------------

    const videoAndPlaylist = contentParent.childNodes[0];

    // console.log(videoAndPlaylist)

    videoAndPlaylist.style.display = "flex";
    videoAndPlaylist.style.flexDirection = "column";
    videoAndPlaylist.style.height = "100%";
    videoAndPlaylist.style.alignItems = "flex-end";

    // ---------------------------------------------

    const videoContainer = videoAndPlaylist.childNodes[0];

    //console.log(videoContainer)

    removeIfExists(videoContainer.childNodes[2]);
    removeIfExists(videoContainer.childNodes[2]);

    // ---------------------------------------------

    const playlistContainer = videoAndPlaylist.childNodes[1];

    //console.log(playlistContainer)

    playlistContainer.style.margin = "5px 0 0 0";
    playlistContainer.style.flexGrow = "1";
    playlistContainer.style.width = "60%";

    playlistContainer.firstElementChild.style.height = "100%";

    // ---------------------------------------------

    const navbar = document.querySelector(".sc-jfJzZe");

    removeIfExists(navbar.childNodes[0]);
    removeIfExists(navbar.childNodes[0]);
    ...
    let contentParentLeft = contentParent.getBoundingClientRect().left;
    let playlistCoordTop = playlistContainer.getBoundingClientRect().top;
    let playlistCoordLeft = playlistContainer.getBoundingClientRect().left;
    let playlistWidth = playlistContainer.getBoundingClientRect().width;
    let playlistHeight = playlistContainer.getBoundingClientRect().height;

    navbar.style.left = `calc(${contentParentLeft}px + 5px)`;
    navbar.style.top = `${playlistCoordTop}px`;
    navbar.style.height = `calc(${playlistHeight}px)`;
    navbar.style.right = `calc(${vw - playlistCoordLeft}px + 5px)`;
    ...
    window.onresize = () => {
        ...
        contentParentLeft = contentParent.getBoundingClientRect().left;
        playlistCoordTop = playlistContainer.getBoundingClientRect().top;
        playlistCoordLeft = playlistContainer.getBoundingClientRect().left;
        playlistWidth = playlistContainer.getBoundingClientRect().width;
        playlistHeight = playlistContainer.getBoundingClientRect().height;
    ...
    document.addEventListener("keyup", (e) => {
        if (e.key == "f") {
            document.querySelector("#bmpui-id-196").click();
        }
    });
}
